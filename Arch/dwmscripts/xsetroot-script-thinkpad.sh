...
while true; do
	...
	xsetroot -name " 1:$(acpi | head -n 1 | cut -d ',' -f2) |  2:$(acpi | tail -n 1 | cut -d ',' -f2) |  $(xbacklight -get | cut -d. -f1)% | $(if [[ "$var1" == 'yes' ]]; then echo 婢; else echo 墳; fi) $(pactl list sinks | grep '^[[:space:]]Volume:' | head -n 1 | tail -n 1 | sed -e 's,.* \([0-9][0-9]*\)%.*,\1,')% | $( date +"%F %R" )"
	sleep 0.5s
done
